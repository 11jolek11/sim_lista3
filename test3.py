...
# set parameters
    ...
    queue_size = 10*9 #size of queue
    is_exp_drop = True # or False
    
    simulation = simulator(lambda_,
                                mu,
                                theta,
                                queue_size,
                                is_exp_drop)
    ...
    sim_pb, sim_pd = simulation.run(num_customers)
    print(f"is_exp_drop:{is_exp_drop}, lambda:{lambda_}, mu:{mu}, theta:{theta} => simulation pb:{sim_pb}\n")
